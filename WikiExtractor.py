...
def normalize(text):
    error1 = re.compile("[가-힣\b][:].*[\n]|\(.*?\)")
    error2 = re.compile("[\w가-힣\b]*[)].*\n")
    html_tag = re.compile("<br>|</br>")
    error3 = re.compile("['‘’~\-/\",“…”:·※&《》〈〉<>\[\]\{\}「」]|(<200b>)")
    next_line1 = re.compile("[가-힣][.]{1}\s")
    next_line2 = re.compile(
        "(?P<sentence>[가-힣])(?P<punc>[.]{1})(?P<sentence2>[0-9A-z가-힣])"
    )
    paragraph = re.compile("[\n]{2,}")
    # tag2=re.compile("##TAG##\n")
    # space=re.compile("[\s][\s]{0,}[\s]")
    delete1 = re.compile("[^\n][가-힣\w\b]+?[$\n]|^[##TAG##].*[$\n]")
    delete2 = re.compile("(?P<punc>[.])(?P<blank>[ ])")
    delete3 = re.compile("\n[ ]")
    final = re.compile("[^\s\n가-힣A-z0-9.]")
    # clean = re.compile("^[\w가-힣ㄱ-ㅎㅏ-ㅣ]")
    text = str(text)
    text = error1.sub("", text)
    text = error2.sub("", text)
    text = tag.sub("##TAG##", text)
    text = html_tag.sub("", text)
    text = error3.sub("", text)
    text = next_line1.sub("\g<0>\n", text)
    text = next_line2.sub("\g<sentence>\g<punc>\n\g<sentence2>", text)
    text = paragraph.sub("", text)
    text = paragraph.sub("", text)
    text = delete1.sub("", text)
    text = delete2.sub("\g<punc>", text)
